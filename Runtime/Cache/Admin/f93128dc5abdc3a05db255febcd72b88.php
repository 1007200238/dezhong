...
<form action="<?php echo U('Psi/contract');?>" method="POST" id="list" class="form-horizontal">
	<!-- 标题栏 -->
	...
	<div class="cf">
	    ...
	    <tbody>
			<?php if(!empty($_list)): if(is_array($_list["info"])): $i = 0; $__LIST__ = $_list["info"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(!empty($vo["code"])): ?><tr>
					...
					<td><?php echo ($vo["purch_price_off"]); ?></td>
					<td><input type="number" style="width:60px;" name="list[<?php echo ($vo["supplier"]); ?>][<?php echo ($i); ?>][num]" value="<?php echo ($vo["num"]); ?>" id="num_<?php echo ($i); ?>" onchange="getval(<?php echo ($i); ?>,1);" class="num"></td>
					<td><input type="number" style="width:60px;" name="list[<?php echo ($vo["supplier"]); ?>][<?php echo ($i); ?>][total]" value="<?php echo $vo['num']*$vo['purch_price_off']; ?>" id="total_<?php echo ($i); ?>" onchange="getval(<?php echo ($i); ?>,2);"></td>
					<input type="hidden" id="price_<?php echo ($i); ?>" value="<?php echo ($vo["purch_price_off"]); ?>" />
					<input type="hidden" name="list[<?php echo ($vo["supplier"]); ?>][<?php echo ($i); ?>][sid]" value="<?php echo ($vo["id"]); ?>" />
				</tr>
				<?php $sum += $vo['num']; ?>
				<?php $total += $vo['num']*$vo['purch_price_off']; endif; endforeach; endif; else: echo "" ;endif; ?>
			<tr>
				<td colspan="7" style="font-weight:bold; text-align:center"> 合 计 </td>
				<td><input name="num" id="num" type="number" style="width:60px;" value="<?php echo ($sum); ?>"></td>
				<td><input name="total" id="total" type="number" style="width:60px;" value="<?php echo ($total); ?>"></td>
			</tr>
			<input type="hidden" name="apply" value="<?php echo ($_list["id"]); ?>" />
			<?php else: ?>
		...
		<div class="fb">
			...
			<input name="note" type="text" class="text input-large" value="<?php echo C('SITE_CONTRACT_NOTE');?>">&nbsp;&nbsp;&nbsp;
	...
	<script type="text/javascript">
	    //导航高亮
	    highlight_subnav('<?php echo U('Psi/apply_list');?>');
	</script>
