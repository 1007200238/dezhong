...
<form action="<?php echo U('Psi/order_return');?>" method="POST" id="list" class="form-horizontal">
	<!-- 标题栏 -->
	...
	<div class="cf">
	    ...
	    <tbody>
			<?php if(!empty($_list)): if(is_array($_list["goods"])): $i = 0; $__LIST__ = $_list["goods"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(!empty($vo["code"])): ?><tr>
					...
					<input type="hidden" value="<?php echo ($vo["out_num"]); ?>" id="num_<?php echo ($i); ?>" />
				</tr>
				<?php $sum += $vo['num']; endif; endforeach; endif; else: echo "" ;endif; ?>
			<tr>
			...
			<input type="hidden" name="pid" value="<?php echo ($_list["uid"]); ?>" />
