...
<body>
	...
	<div class="cf">
		<div class="fl">
			...
			<select name="year" id="years" class="change">
				...
				<?php if(is_array($_sel["year"])): $i = 0; $__LIST__ = $_sel["year"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>" <?php echo ($vo); ?>><?php echo ($key); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			</select> 年
			<select name="month" id="month" class="change">
				<option value="0">请选月份</option>
				<?php if(is_array($_sel["month"])): $i = 0; $__LIST__ = $_sel["month"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($i); ?>" <?php echo ($vo); ?>><?php echo ($i); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			</select> 月
			&nbsp;&nbsp;&nbsp;产品分类： 
	...
	<script src="/Public/static/highcharts/modules/exporting.js"></script>
    ...
    $(document).ready(function () {
		...
		$('#years,#month,#cat,#sup').change(function(){
			...
			function(datas){
				...
				var years = $('#years').val();
				if (years!="0") { 
					column_1.setTitle({text:"<b>"+years+"年产品销售数据排行柱状图</b>"}); 
				}else{
					column_1.setTitle({text:"<b>本年产品销售数据排行柱状图</b>"});
				}	
	   		});				
		});
		...
		$('#column_1').highcharts({
            ...
            title: {
                text: ' <b><?php echo date("Y",strtotime("-1 month")); ?>年产品销售数据排行柱状图</b> '
            },
            subtitle: {
                text: '（产品分布，筛选时间、产品分类）',
                style: { fontSize: '15px' },
            ...
            tooltip: {
                pointFormat: '此月份销量: <b>{point.y:.1f} </b>',
            },
            series: [{
                name: '项目统计',
                data: <?php echo ($data); ?>,
