...
<form action="<?php echo U('Psi/in_do');?>" method="POST" id="list" class="form-horizontal">
	<!-- 标题栏 -->
	...
	<div class="cf">
		<div class="fl">
            ...
            <input name="no" type="text" class="text large" placeholder="请填写入库单号" value="<?php echo C('SITE_IN_NO');?>"/>&nbsp;&nbsp;&nbsp;
	    ...
	    <tbody>
			<?php if(!empty($_list)): if(is_array($_list["info"])): $i = 0; $__LIST__ = $_list["info"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(!empty($vo["code"])): ?><tr>
					...
					<input type="hidden" value="<?php echo ($last); ?>" id="num_<?php echo ($i); ?>" />
				</tr>
				<?php $sum += $vo['num']; ?>
				<?php $sum_in += $vo['in_num']; ?>
				<?php $sum_last += ($vo['num'] - $vo['in_num']); endif; endforeach; endif; else: echo "" ;endif; ?>
			<tr>
				<td colspan="5" style="font-weight:bold; text-align:center"> 合 计 </td>
				<td><?php echo ($sum); ?></td>
				<td><?php echo ($sum_in); ?></td>
				<td><span style="color:red"><?php echo ($sum_last); ?></span></td>
				<td><span id="sum_do">0</span></td>
			</tr>
			<input type="hidden" name="supplier" value="<?php echo ($_list["supplier"]["id"]); ?>" />
		...
		<div class="fb">
			...
			<input name="note" type="text" class="text input-large" value="<?php echo C('SITE_IN_NOTE');?>">&nbsp;&nbsp;&nbsp;
...
document.onkeydown = function()
{
	...
	if (oEvent.keyCode == 65 && oEvent.ctrlKey) {
		$.post("<?php echo U();?>", {dos: '123'}, function(data) {
	...
	<script type="text/javascript">
	    //导航高亮
	    highlight_subnav('<?php echo U('Psi/in_apply');?>');
	</script>
