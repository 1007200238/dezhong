...
<body>
	...
	<div class="cf">
		<div class="fl">
			<button class="btn ajax-post confirm" url="<?php echo U('Psi/order_del');?>" target-form="ids">批量删除</button>
        </div>
		<!-- 高级搜索 -->
		<div class="search-form fr cf">
			<div class="sleft">
				<div class="drop-down">
					<span id="sch-sort-txt" class="sort-txt" data="<?php echo ($type); ?>"><?php if(I('type') == ''): ?>订购单编号<?php else: echo get_status_type($type); endif; ?></span>
				...
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Psi/order');?>"><i class="btn-search"></i></a>
			</div>
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><a href="<?php echo U('Psi/order_info');?>&id=<?php echo ($vo["id"]); ?>"><?php echo ($vo["order_id"]); ?></a></td>
			<td>￥<?php echo ($vo["money"]); ?></td>
			<td><?php echo (get_username($vo["uid"])); ?></td>
			<td><?php echo ($vo["name"]); ?></td>
			<td><?php echo ($vo["tel"]); ?></td>
			<td><?php echo (date("Y-m-d H:i",$vo["date"])); ?></td>
			<td><?php echo (get_status_type($vo["status"],"order")); ?></td>
			<td>
				<?php if ($uid == 1) { ?>
					<a href="<?php echo U('Psi/order_info');?>&id=<?php echo ($vo["id"]); ?>">详细</a> |
					<a href="<?php echo U('Psi/order_process');?>&id=<?php echo ($vo["id"]); ?>">添加流程</a>
					<?php if ($vo['status'] < 3) { ?>
					 	| <a href="<?php echo U('Psi/order_edit');?>&id=<?php echo ($vo["id"]); ?>">编辑</a> 
					<?php } ?>
					<?php if ($vo['status'] < 6 && $vo['status'] > 2) { ?>
					    | <a href="<?php echo U('Psi/order_return');?>&id=<?php echo ($vo["id"]); ?>">退货</a> 
					<?php } ?>
				<?php } else { ?>
					<a href="<?php echo U('Psi/order_info');?>&id=<?php echo ($vo["id"]); ?>">详细</a> 
					<?php if ($vo['status'] < 2) { ?>
					| <a href="javascript:void(0);" onclick="confirm_order(<?php echo ($vo["id"]); ?>);">确认订单</a> 
					<?php } ?>
				<?php } ?>
			</td>		
	...
	function confirm_order(id) {
		$.post("<?php echo U('Psi/order');?>", {id: id}, function(data) {
			alert(data);
			location.reload();
		});
	}
