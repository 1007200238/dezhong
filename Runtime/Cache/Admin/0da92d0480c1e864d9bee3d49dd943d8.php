...
<style>
	...
	<div class="cf">
		<div class="fl">
			<!-- <button class="btn ajax-post confirm" url="<?php echo U('Psi/contract_del');?>" target-form="ids">批量删除</button> -->
        </div>
        <!-- 高级搜索 -->
		<div class="search-form fr cf">
			<div class="fl">
				<select name="category" id="category" onchange="gourl('category');" class="change">
					...
					<?php if(is_array($_sel["category"]["_"])): $i = 0; $__LIST__ = $_sel["category"]["_"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if(I('category')==$vo['id']){ ?> selected <?php } ?> ><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</div>
			<div class="fl">
				<div class="controls">
                    <select name="supplier" class="change" id="supplier" onchange="gourl('supplier');">
	                    ...
	                    <?php if(is_array($_sel["supplier"])): $i = 0; $__LIST__ = $_sel["supplier"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if(I('supplier')==$vo['id']){ ?> selected <?php } ?> ><?php echo ($vo["company"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>            
                </div>
			</div>
			<div class="sleft">
				<div class="drop-down">
					<span id="sch-sort-txt" class="sort-txt" data="<?php echo ($type); ?>"><?php if(I('type') == ''): ?>产品编码<?php else: echo get_status_type($type); endif; ?></span>
				...
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Psi/data_1');?>"><i class="btn-search"></i></a>
			</div>
	    ...
	    <thead>
	        <tr>
				...
				<th class="">联系方式</th>
				<th class="">email</th>
				<th class=""><a href="#" class="asc" onclick="dosortSp($(this),'num');">购入量</a></th>
				<th class=""><a href="#" class="asc" onclick="dosortSp($(this),'total');">购入额</a></th>
			</tr>
	    </thead>
	    <tbody>
			<?php if(!empty($data)): if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				...
				<td><?php echo ($vo["truename"]); if(empty($vo["truename"])): ?>尚未填写<?php endif; ?></td>
				<td><?php echo ($vo["company"]); if(empty($vo["company"])): ?>尚未填写<?php endif; ?></td>
				<td><?php echo ($vo["mobile"]); if(empty($vo["mobile"])): ?>尚未填写<?php endif; ?></td>
				<td><?php echo ($vo["email"]); if(empty($vo["email"])): ?>尚未填写<?php endif; ?></td>
