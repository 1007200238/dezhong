...
<form action="<?php echo U('');?>" method="POST" id="list" class="form-horizontal">
	<!-- 标题栏 -->
	...
	<div class="cf">
	    ...
	    <tbody>
			<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(!empty($vo["code"])): ?><tr>
					...
					<td><?php echo ($vo["ware_num"]); ?></td>
					<td><input type="number" style="width:60px;" name="info[<?php echo ($i); ?>][num]" value="0" class="num"></td>
					<td><input type="number" style="width:60px;" id="total_<?php echo ($i); ?>" value="0" disabled></td>
					...
					<input type="hidden" value="<?php echo ($vo["ware_num"]); ?>" id="num_<?php echo ($i); ?>" />
					<input type="hidden" value="<?php echo ($vo["purch_price_off"]); ?>" id="price_<?php echo ($i); ?>" />
				</tr>
				<?php $sum_in += $vo['ware_num']; endif; endforeach; endif; else: echo "" ;endif; ?>
			<tr>
	...
	<script type="text/javascript">
	    //导航高亮
	    highlight_subnav('<?php echo U('Psi/free_return');?>');
	</script>
