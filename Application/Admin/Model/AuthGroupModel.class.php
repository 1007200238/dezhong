...
class AuthGroupModel extends Model {
     ...
     * 返回用户所属用户组id
     * @param  int    $uid 用户id
     * @param  int    $type 返回类型 1：返回用户组id  2：如果用户组为加盟商返回他们的userid
     * @return int    用户所属的用户组
     */
    static public function getGroupid($uid,$type=1){
        static $groups = array();
        if (isset($groups[$uid]))
            return $groups[$uid];
        $prefix = C('DB_PREFIX');
        $user_groups = M()
            ...
            ->select();
        $groups[$uid]=$user_groups?$user_groups:array();
        $group_id = $groups[$uid]['group_id'];
        if ($type == 2 && $group_id == 2) {
            //如果是加盟商，则返回他们各自的uid（仓库id）
            $group_id = $groups[$uid]['uid'];
        }
